...
class JpNtaSpider(SmartSpider):
    name = 'jp_nta'
    country_code = 'JPN'
    country = '日本'
    language = 'ja'
    source_timezone = 'Asia/Tokyo'
    allowed_domains = ['nta.go.jp']
    start_urls = ['https://www.nta.go.jp/information/news/index.htm']
    ...
    def parse(self, response):
        ...
        for a_sel in all_links:
            ...
            # Skip pdf/document links to avoid NotSupported exceptions
    ...
    def parse_detail(self, response):
        if not isinstance(response, scrapy.http.TextResponse):
            self.logger.warning(f"Skipping non-text response: {response.url} (Type: {type(response)})")
            return
        ...
        item['author'] = 'National Tax Agency'
