...
class JpNikkeiXTrendSpider(SmartSpider):
    name = 'jp_nikkei_xtrend_subscription'
    ...
    allowed_domains = ['xtrend.nikkei.com']
    start_urls = ['https://xtrend.nikkei.com/atcl/contents/theme/?nxr_e=B0041%E3%83%9E%E3%83%BC%E3%82%B1&i_cid=nbpnxr_navi']
    ...
    def parse(self, response):
        ...
        for a_sel in all_links:
            ...
            if any(x in url for x in ['/theme/', '/search/', '/mypage/', '/info/', '/ranking/']):
                continue
            ...
            # Route to parse (listing) if it is a series index page, otherwise to parse_detail
            norm_url = url.split('?')[0].rstrip('/')
            if re.search(r'/contents/\d{2}/\d{5}$', norm_url):
