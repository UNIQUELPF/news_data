...
class IndiaMoneycontrolBizSpider(SmartSpider):
    name = "india_moneycontrol_biz"
    country_code = 'IND'
    country = '印度'
    language = 'en'
    allowed_domains = ["moneycontrol.com"]
    target_table = "ind_moneycontrol_biz"
    ...
    async def start(self):
        ...
        # Explicitly passing cookies in the first request to ensure GDPR bypass.
    ...
    def parse_list(self, response):
        ...
        # Extract links from the news list
        ...
        for link in set(links):
            # STRICT FILTERING: 
            # 1. Must be a business news link
            # 2. Must NOT be an economy link (for this spider)
            # 3. Must be an ACTUAL ARTICLE (ends in .html)
            # 4. Must NOT be a pagination link (contains /page-)
            if '/news/business/' in link and '/news/business/economy/' not in link:
        ...
        if (new_links_found > 0) or (self.full_scan and valid_article_links):
            ...
            next_url = f"https://www.moneycontrol.com/news/business/page-{next_page}/"
