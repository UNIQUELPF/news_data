...
class KuwaitBaseSpider(SmartSpider):
    country_code = "KWT"
    country = "科威特"
    language = "en"
    ...
    def _build_item(self, response, title, content, publish_time, author, language, section):
        ...
        # Prefer ContentEngine output when it provides longer/richer extraction
        ce_plain = (content_data.get("content_plain") or "").strip()
        if ce_plain and len(ce_plain) > len(content or ""):
            content = ce_plain
    ...
    def _extract_content(self, response, selectors):
        ...
        for selector in selectors:
            ...
            for unwanted in root.select(
                ...
                ".share, .breadcrumb, .social-share, .related-posts"
            ):
                unwanted.decompose()
            parts = []
