...
class AustraliaAfrSpider(AustraliaBaseSpider):
    name = "australia_afr"
    ...
    def parse_detail(self, response):
        ...
        author_data = (data or {}).get("author")
        ...
        section = self._clean_text((data or {}).get("articleSection")) or self._section_from_url(response.url)
    ...
    def _looks_like_article(self, url):
        ...
        return bool(re.search(r"-\d{8}-p[0-9a-z]+$", path))

    def _section_from_url(self, url):
        for section in ("companies", "policy", "markets"):
            if f"/{section}/" in url:
                return section
        return "business"
    ...
    def _extract_article_schema(self, response):
        for raw in response.css('script[type="application/ld+json"]::text').getall():
            ...
            for candidate in candidates:
                if isinstance(candidate, dict) and candidate.get("@type") in {"NewsArticle", "Article", "ReportageNewsArticle"}:
                    return candidate
                graph = candidate.get("@graph") if isinstance(candidate, dict) else None
                if not isinstance(graph, list):
                    continue
                for entry in graph:
                    if isinstance(entry, dict) and entry.get("@type") in {"NewsArticle", "Article", "ReportageNewsArticle"}:
                        return entry
        return None
