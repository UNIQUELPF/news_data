...
class TimorLesteInetlSpider(TimorLesteBaseSpider):
    name = "timor_leste_inetl"
    ...
    def parse_detail(self, response):
        ...
        yield self._build_item(
            ...
            author=self._clean_text(response.xpath("//meta[@name='author']/@content").get()) or "INETL",
            language="en",
            section="statistics",
        )
    ...
    def _extract_content(self, response, title):
        ...
        root = soup.select_one("article") or soup.select_one(".entry-content") or soup.select_one("main")
        if not root:
            return ""
        for unwanted in root.select("script, style, nav, footer, header, aside, form, .sharedaddy, .wp-block-buttons"):
            unwanted.decompose()
