...
class KyrgyzstanBaseSpider(scrapy.Spider):
    target_table = ""
    ...
    def _extract_pdf_text(self, url, max_pages=6):
        try:
    ...
    def _extract_content(self, response, selectors):
        ...
        for selector in selectors:
            ...
            if not root:
                continue
            for unwanted in root.select("script, style, nav, footer, header, aside, form, .share, .related, .breadcrumb"):
                unwanted.decompose()
            parts = []
