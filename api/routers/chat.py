...
async def _stream_chat_response(session_id: str, user_id: str, message: str, think_mode: bool) -> AsyncGenerator[str, None]:
    ...
    system_prompt = (
        ...
        '# 核心原则\n'
        '1. **数据源认知**：通过 `search_news_articles` 检索到的文章，是系统自动爬取自全球各大主流媒体（如新华网、联合早报、东方日报等）的新闻数据。它们**绝对不是用户上传的文档**，用户并没有上传任何文件。请将检索到的文章统称为"系统数据库中收录的新闻文章"。\n'
        ...
        '3. **过滤无关检索**：如果调用检索工具返回了与用户问题不相关的新闻（这是向量检索的常见现象），你**绝对不能**强行把它们联系到用户的问题上，更不能为了套用这些文章而编造谎言（例如谎称"这是用户上传的参考资料"）。如果检索到的内容不相关，请忽略它们，并坦诚告知"系统数据库中暂未收录与该问题相关的政经新闻"。\n'
